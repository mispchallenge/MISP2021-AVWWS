...
def wpe_worker(wav_scp, data_root='MISP_data_root', output_root='MISP_data_out_root', processing_id=None, processing_num=None):
    ...
    for wav_idx in range(len(wav_lines)):
        ...
        if processing_token:
            ...
            os.system('mkdir -p {}'.format(os.path.join(output_root,'/'.join(wav_list[0].split('/')[-6:-1]))))
            signal_list = []
            for f in wav_list:
                _, data = wf.read(f)
...
if __name__ == '__main__':
    ...
    output_root = os.path.join(root_path,'WPE')
    wav_scp = os.path.join(root_path,'wpe.scp')
