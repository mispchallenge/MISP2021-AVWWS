...
def run_beamforming(wavscp,output_root,root_path,channel_dir='./channels_name_dir/',conf_dir='./configure_dir/'):
    ...
    for i,file in enumerate(wav_file_list):
        ...
        file_channel_name.extend([file_name.replace('_0.wav','_{}.wav'.format(i)) + ' ' if i != channel_num-1 else file_name.replace('_0.wav','_{}.wav'.format(i)) for i in range(0,channel_num)])

        channel_files = channel_dir + 'channels_conf'
        source_dir = '/'.join(file_split[:-1])
        result_dir = os.path.join(output_root, '/'.join(file_split[-5:-1])  + '/')
