...
class myDataset_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lip_feature_ori = np.squeeze(np.load(video_path), axis=3)
        
        T = lip_feature_ori.shape[0]
        lip_feature = np.empty_like(lip_feature_ori)
        for i in range(T):
            lip_feature[i] = np.array(augment(Image.fromarray(np.uint8(lip_feature_ori[i]))))
...
class myDataLoader_train(DataLoader):
